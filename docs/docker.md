...
If starting from scratch and your host machine is Ubuntu 22.04, you can use accomplish all of the above with the convenience scripts `scripts/deployment/docker/install_docker_ubuntu22.sh` and `scripts/deployment/docker/install_nvidia_container_toolkit.sh`.

Build the Docker image and start the container with the following command:
```bash
docker compose -f scripts/deployment/docker/compose.yml up --build
```
