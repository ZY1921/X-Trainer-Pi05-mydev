...
import openpi.policies.atom_policy as atom_policy
...
@dataclasses.dataclass(frozen=True)
class LeRobotXTrainerDataConfig(DataConfigFactory):
    # If true, convert the 12 joint dimensions (excluding both grippers) to delta actions.
    use_delta_joint_actions: bool = True
    # If provided, will be injected into the input data if the "prompt" key is not present.
    default_prompt: str | None = None

    # Repack transforms for LeRobot datasets where action is stored in the "action" key.
    repack_transforms: tyro.conf.Suppress[_transforms.Group] = dataclasses.field(
        default=_transforms.Group(
            inputs=[
                _transforms.RepackTransform(
    ...
    # Action key used by LeRobot datasets.
    action_sequence_keys: Sequence[str] = ("action",)
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        if self.use_delta_joint_actions:
            # Keep grippers absolute, convert both 6-DOF joint blocks to deltas.
            delta_action_mask = _transforms.make_bool_mask(6, -1, 6, -1)
            data_transforms = data_transforms.push(
                inputs=[_transforms.DeltaActions(delta_action_mask)],
                outputs=[_transforms.AbsoluteActions(delta_action_mask)],
...
class LeRobotAtomDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        data_transforms = _transforms.Group(
            inputs=[atom_policy.AtomInputs()],
            outputs=[atom_policy.AtomOutputs()],
        )
        if self.use_delta_joint_actions:
            # State/action order: left_arm(7), left_hand(6), right_arm(7), right_hand(6), head(2).
            # Convert only arm joints to deltas.
            delta_action_mask = _transforms.make_bool_mask(7, -6, 7, -8)
            data_transforms = data_transforms.push(
                inputs=[_transforms.DeltaActions(delta_action_mask)],
                outputs=[_transforms.AbsoluteActions(delta_action_mask)],
            )
...
_CONFIGS = [
    ...
    # This variant expects xtrainer norm stats under "assets/xtrainer" in the checkpoint.
    # Use this with xtrainer-specific fine-tuned checkpoints.
    TrainConfig(
        name="pi05_xtrainer_custom",
        model=pi0_config.Pi0Config(pi05=True),
        data=LeRobotXTrainerDataConfig(
            assets=AssetsConfig(asset_id="xtrainer"),
        ),
    ),
    # Fine-tuning config for custom XTrainer datasets.
    # - Expects xtrainer-specific stats under assets/xtrainer.
    # - For JAX training, initializes from pi05_base via weight_loader.
    # - For PyTorch training, pass --pytorch-weight-path to a converted checkpoint.
    TrainConfig(
        name="pi05_xtrainer_finetune",
        model=pi0_config.Pi0Config(pi05=True),
        data=LeRobotXTrainerDataConfig(
    ...
    TrainConfig(
        name="pi05_xtrainer_lora_finetune",
        model=pi0_config.Pi0Config(
            pi05=True,
    ...
    # LoRA r=64 fine-tuning config for custom XTrainer datasets.
    TrainConfig(
        name="pi05_xtrainer_lora_r64_finetune",
        model=pi0_config.Pi0Config(
            pi05=True,
    ...
    TrainConfig(
        name="pi05_atom_lora_r64_finetune",
        model=pi0_config.Pi0Config(
            pi05=True,
            paligemma_variant="gemma_2b_lora_r64",
            action_expert_variant="gemma_300m_lora_r64",
